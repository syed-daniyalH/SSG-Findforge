...
export default async function TechnologyChildDetailPage({
  params,
}: {
  ...
  const childItem = technologyItem?.children.find(
    (item) => item.slug?.toLowerCase() === childSlug.toLowerCase().trim(),
  );

  if (!technologyItem || !childItem) {
    notFound();
  }

  return (
    <div className="technology-child-page">
      <section className="max-w-7xl mx-auto px-4 sm:px-6 md:px-10 lg:px-30 my-10 pt-20">
        <div className="max-w-3xl space-y-6">
          ...
          />
          <h1 className="text-primary">{childItem.name}</h1>
          <p className="text-neutral-dark">
            This page is resolved from the shared Techionik CMS menu tree. When
            a dedicated detail API becomes available, the frontend can plug it
            into this route without changing the public URL.
          </p>
